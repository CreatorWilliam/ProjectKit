...
private extension API {
  ...
  func debugLog(_ json: JSON) {
    ...
    let api = self
    print(api.path)
    if let header = api.headerFieldParameters { print(JSON(header))}
    if let query = api.queryParameters { print(JSON(query)) }
    if let body = api.bodyParameters { print(JSON(body)) }
    print(json)
    #endif
  }
